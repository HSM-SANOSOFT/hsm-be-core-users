...
export class DatabaseService {
  ...
  async getUserData(userCode: string) {
    const query = `SELECT P.USUARIO, P.CODIGO, P.ESTADO_DE_DISPONIBILIDAD, P.UBICACION, P.CARGO, P.ESPPRS_CODIGO, P.NOMBRES, P.APELLIDOS, P.TELEFONO, P.EMAIL, P.DIRECCION, P.SEXO, P.SENESCYT, C.RV_MEANING AS CARGONAME, E.ESPECIALIDAD AS ESPECIALIDADNAME FROM PERSONAL P LEFT JOIN CG_REF_CODES C ON P.CARGO = C.RV_LOW_VALUE AND C.RV_DOMAIN = 'CARGO' LEFT JOIN ESPECIALIDAD_PERSONAL E ON P.ESPPRS_CODIGO = E.CODIGO WHERE P.CODIGO = '${userCode}'`;
